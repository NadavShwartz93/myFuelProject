...
public class PurchasesReportQuery {

	/**
	 * The function execute queries for the Purchases report.
	 * later the function builds the report and save it to the DB.
	 * The method the full report.
...
//---------			
		...
		tempString = "SELECT fp.FK_fuelType, SUM(fp.fuelPurchasePrice), COUNT(*) FROM myfueldb.fuelpurchases fp, myfueldb.gasstation gs WHERE fp.FK_gasStationId = gs.gasStationId AND gs.FK_gasStationManager = '" + oldMsg.get(0) + "' AND fp.purchasesDay >= '" + begining + "' AND fp.purchasesDay <= '" + end + "' GROUP BY fp.FK_fuelType;";
 		rs=stmt.executeQuery(tempString);
		...
		try
		{
		tempString = "INSERT INTO `myfueldb`.`quarterreports` (`QuarterNumber`, `qr_Year`, `reportType`, `FK_gasStation_Id`, `theFile`) VALUES ('" + quarter + "', '" + year + "', 'purchased_report', '" + stationId + "', '" + report + "');";
		...
		catch (SQLException e)
		{
			...
			{
				tempString = "SELECT theFile FROM quarterreports WHERE reportType = 'purchased_report' AND FK_gasStation_Id = '" + stationId + "' AND qr_Year = '" + year + "' AND QuarterNumber = '" + quarter + "';";
	...
	private static String createReport(ArrayList<Object> reportData) {
		...
		for (int i = 1; i < reportData.size(); i++) {
			...
			strTemp += String.format("%s:\n\tNumber of Purcheses: %s\n\tTotal revenue: %s$\n\n", row.get(0), row.get(2), String.format("%.2f", row.get(1)));
		}
		strForFile += strTemp;
